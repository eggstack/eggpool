...
- **Dispatch isolation**: local preparation, request construction/serialization, and client-facing response adaptation failures are terminal local errors with no provider retry or penalty. Only typed HTTPX transport failures may retry, only across distinct accounts before `downstream_started`, and cleanup converges before reselection. The existing `1 + max_retries_before_stream` ceiling is also bounded by distinct eligible accounts; a truncated traversal records `attempt_ceiling_reached`.
...
- **Process model**: supervisor + Granian worker (`workers=1`), daemon mode (`--verbose` for foreground). `runtime_threads=1` is required; other values fail configuration validation. `database_worker_threads=1` is the lean default; `2` explicitly enables a separate stats connection. Readiness probe is process-owned and disabled by default.
- **Lean defaults**: ordinary onboarding binds to loopback unless an interactive LAN choice selects `0.0.0.0`; low-wear analytics, provider pools of 16/4, and background outbound pools of 8/2 are the baseline. Model-info, routing traces, detailed spans, readiness writes, event-loop lag, DNS caching, automatic backups, dispatch writing, and the background PyPI checker are opt-in. Disabled features are represented as `None` and do not construct clients, writers, queues, recorders, callbacks, or tasks.
- **Runtime generations**: `RuntimeManager` owns active/retiring generation slots. Lease acquisition is fail-closed: `RuntimeManagerLeaseExhaustedError` → HTTP 503. Staged reload swap: `stage()` → `commit()`/`rollback()` → `finalize_retirement()`. `RuntimeGenerationFactory.prepare()` is the shared startup/rehash construction boundary: candidate builds create only enabled generation-owned resources, never process-owned task duplicates, and `candidate.abort()` closes only resources actually registered, in reverse order. `RequestFinalizationSupervisor` is generation-owned: each accepted retained job holds one terminal retirement reference until durable and required runtime convergence; retirement closes only after both request leases and terminal references reach zero. A live unresolved reference triggers the existing fatal worker handler; bounded process shutdown may abandon references for startup repair.
...
- **Health management**: `src/eggpool/health/` — `HealthManager` circuit breaker, per-account health tracking, bounded 1,800-second nonterminal backoff, scoped success/expiry recovery, validated-rehash authentication reset, scoped model quarantine, and `DatabaseWritableProbe` (real SQLite write probes, cached for `/readyz`). Durable backoff rows are restart hints only; hydration ignores malformed/expired/unknown rows and never turns runtime model absence into account-wide suppression. Model-quarantine hydration is fail-closed: an empty successful read is distinct from a read/conversion failure, and failed startup/rehash preparation never publishes unknown quarantine state.
- **Request finalization**: `RequestFinalizationJob` keyed by `(proxy_request_id, attempt_id)` carries an explicit `AttemptRuntimeLease` made from publication facts. The generation-owned `RequestFinalizationSupervisor` is the sole in-process retry scheduler, using one bounded timer and an execution-time absolute retry-age deadline. Each accepted job retains its owning generation exactly once; duplicates and retries do not multiply references, and scalar diagnostic history does not retain the generation. Capacity rejects before reference acquisition using `FinalizationData.downstream_started`, never `bytes_emitted`; after handoff it records a bounded diagnostic and leaves durable work for repair. Lease-owned usage, health, and account-runtime obligations remain active when durable finalization observes an already-terminal request. `DurableFinalizationResult` reports only durable request/attempt/reservation and cost facts; `FinalizationResult` separately reports durable convergence, live quota/active-count and health/probe results, plus runtime cleanup, projecting completed lease components while retry-pending and clearing transient retry metadata when the lease converges.
- **Retained terminal commands**: one generation-owned `RequestFinalizationSupervisor` stores selected request finalization, failed-attempt cleanup, and post-commit claim compensation. One kind-qualified identity, global capacity, retry scheduler, diagnostics surface, and shutdown drain cover all command kinds. Compatible duplicates join; conflicting identity/kind submissions fail closed. The coordinator submits and joins but owns no retained registry.
- **Runtime accounting**: active requests are never reclaimed by row age, read timeout, or missing usage. Startup crash recovery repairs durable pending rows because the prior process cannot still own them; normal runtime cleanup is owned by the selected attempt's retained terminal owner.
...
- **Failure effects and quarantine**: `classify_failure_effects()` returns one immutable retry/effects decision from bounded response evidence. The decision is carried through retry, retained attempt cleanup, finalization, health, circuit, backoff, and quarantine work. Effect progress is scoped to `(proxy_request_id, attempt_id)` and retired with its retained owner; `HealthManager.record_failure()` owns the single circuit-failure transition and request-local outcomes release probes without provider penalties. `ModelQuarantine` remains a bounded state machine with corroboration before terminal withdrawal. Authoritative reappearance clears the exact durable provider/account/canonical/upstream/protocol key before in-memory suppression or matching backoff; durable failure preserves suppression and remains local database evidence.
