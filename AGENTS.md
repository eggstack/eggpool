...
- **Request finalization**: `RequestFinalizationJob` keyed by `(proxy_request_id, attempt_id)`. `RequestFinalizationSupervisor` drains on shutdown. Retryable attempts use coordinator-retained cleanup with 128-entry capacity; cleanup proves durable transition before next account selection.
