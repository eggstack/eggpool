...
- **Model capabilities**: protocol-neutral `ThinkingCapability` / `ModelCapabilities` schema in `src/eggpool/catalog/capabilities.py` with deterministic merge (defaults → provider catalog → global overrides → provider-scoped overrides), aggregate semantics (all-supported/all-unsupported/mixed/conflicting), compact serialization for `/v1/models` under `eggpool.capabilities`, and request-level helpers (`client_requests_thinking()`, `has_thinking_support()`). Protocol compatibility alone does not imply thinking support. Config overrides: `ThinkingCapabilityOverrideConfig` in `src/eggpool/models/config.py` provides operator-controlled model capability overrides. Global overrides at `[model_capabilities."<model_id>".thinking]` and provider-scoped overrides at `[providers.<id>.model_capabilities."<model_id>".thinking]`. Provider-scoped overrides take precedence over global. See `architecture/README.md` § Model Capabilities.
