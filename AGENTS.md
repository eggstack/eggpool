...
- **Configuration changes currently require a service restart** for the running process; live reload is not yet wired up. `eggpool rehash` validates the new config (running the same validation contract as `eggpool check-config`) but reports that the live control plane is unavailable and never implicitly restarts; use `eggpool restart` to apply changes.
