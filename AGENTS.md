...
- **Dashboard**: server-rendered HTML, Chart.js v4, grouped timeseries, CSS tooltips. Dashboard HTML pages use a 30s in-memory cache (`DashboardTelemetry`) for heavy aggregate queries. Telemetry exposes p50/p95 render times and slowest route under `/api/stats/runtime`.
...
- **Cache & request-shaping diagnostics**: dashboard `/cache` page renders provider cache counters, request segmentation, compression (observe-mode analysis and safe-mode details collapsed by default), policy overrides, native cache preservation, EggPool cache annotations, tuning suggestions, and routing isolation. The `/runtime` page shows only a lightweight Cache & request shaping relocation panel. `QuotaFairScorer` does NOT consume cache, compression, synthetic-cache, or tuning fields — routing stays load-based (request count + token count + active count + health). `tests/unit/test_routing_guardrails.py` pins the invariant. The top `/cache` summary carries six operator-facing cards: Request changes, Provider cache counters, EggPool cache annotations, Safety guardrail, Tuning suggestions, Routing isolation. Quiet default renders show `Clean` / `Isolated` (not raw `reporting_only` or `—`); raw modes survive in subtext/details. Provider cache counters and EggPool cache annotations are separate cards and are explicitly NOT conflated — missing provider cache counters are not cache misses. The advanced diagnostics disclosure is server-decided by `CacheAdvancedState` (`src/eggpool/dashboard/render.py`) and auto-opens for compression warnings / failed fallbacks / stable-prefix mismatches, policy warnings, EggPool annotation warnings or applied count, segmentation parse failures, tuning recommendations or overrides, routing guardrail violations, and transcoding loss warnings. `tests/unit/test_dashboard_cache_page.py` pins the corrected semantics.
...
- **Background tasks**: `src/eggpool/background/` manages retention cleanup, periodic tasks, and startup crash recovery via `TaskSupervisor`. `TaskSupervisor.register(...)` covers daemon-style long-lived coroutines; `TaskSupervisor.register_periodic(...)` covers supervisor-owned periodic scheduling around a one-shot tick factory and records `mode`, `last_tick_started_at`, `last_tick_completed_at`, `last_tick_duration_ms`, `next_run_at`, `overdue_seconds`, `success_count`, `failure_count`, `consecutive_failure_count`, and `last_error_class` so healthy sleeping loops no longer render as ``overdue`` (the legacy `last_started_at + interval_s` math is gone — see `plans/background-task-overdue-remediation.md`). First-tick semantics: default callers sleep `interval_s` before the first tick; `initial_delay_s` overrides the first-tick delay; `run_immediately=True` fires the first tick without delay. `run_immediately` and `initial_delay_s` are mutually exclusive (`ValueError` if both supplied). Overdue detection uses a 25%-of-interval grace band (clamped to 5-60 s). Background task summary (`registered` / `running` / `failed` / `overdue` / `last_error_count`) is exposed under `/api/stats/runtime` via `background_task_summary`. The lifespan shutdown path stops the supervisor first (`stop_all()`) so no new ticks arrive, then performs the final `metrics_coalescer.flush(reason="shutdown")` to drain buffered analytics. `MetricsWriteCoalescer.flush()` is cancellation-safe: if the flush task is cancelled mid-DB-write, the buffer snapshot is merged back so no events are silently dropped. Legacy `_finalize_stale_requests`, `_prune_health_disabled_models_loop`, and `_catalog_refresh_loop` `while True` wrappers are retained as `# pyright: ignore[reportUnusedFunction]` legacy entry points so existing tests still drive the seam-decomposed helpers (`_finalize_stale_requests_once`, `_prune_health_disabled_models_once`, `_catalog_refresh_once`).
...
- **Performance optimization (Phases 0–5)**: correctness-preserving hot-path pass. Phase 0 reuses transcode preflight via `PreparedTranscode` (`src/eggpool/transcoder/prepared.py`) with debug observability on `PreparedTranscodeDiagnostics` (`available`, `reused`, `recompute_reason`). Phase 0.5 skips segmentation when no compression/cache/observability features are active (`should_segment_request` in `src/eggpool/transcoder/segmentation_guard.py`) — gating is driven by the resolved effective compression policy, not the raw global config. Phase 0.6 computes routing in a single `RoutingPlan` pass (`Router.build_routing_plan()` in `src/eggpool/routing/router.py`) instead of double-calling `get_eligible_account_names()` + `select_accounts_for_failover()` — this is the authoritative selection path with no fallback to the legacy `select_accounts()`. Phase 0.7 adds configurable `[routing.trace]` mode (`all`/`sampled`/`off`) with deterministic request-id sampling at write time. Benchmarks: `tests/perf/`.
- **Hot-path dispatch and compression optimization (Phases 1–5)**: Phase 1 adds fine-grained dispatch spans via `DispatchSpanRecorder` (200-sample window) with ~22 `SPAN_*` constants in `runtime_dispatch.py`; spans available via `/api/stats/runtime` `dispatch_spans`. Phase 2 removes duplicate safe-mode work: safe-mode observation is derived from a single `apply_safe_compression()` pass (`SafeModeObservation` duck-types `CompressionObservation`), so the separate `analyze_compression()` call is skipped in safe mode. Phase 3 uses `_copy_with_replacements()` path-level copy-on-write in safe compression; no-op requests forward the original payload without deep-copy. Phase 4 uses single-pass `_build_result()` aggregation in segmentation, avoids representative text allocation for zero-signal segments, and precomputes `estimated_reservation_tokens`, `estimated_context_input_tokens`, and `thinking_requirement` on `ProxyRequestContext`. Phase 5 narrows `_select_lock` to correctness-critical work only and reduces routing trace pressure via `[routing.trace].mode`.
- **Hot-path corrective polish**: closure pass over the Phase 1–5 landing. Selection lock wait/held spans (`SPAN_SELECTION_LOCK_WAIT`, `SPAN_SELECTION_LOCKED`) record exactly one sample per attempt via explicit `record_ns()` calls after the lock exits — there is no `_maybe_span` placeholder for the lock, so the wait/locked p50/p95 metrics are not polluted by near-zero or double-sampled values. `CompressionResult` carries applier-derived `candidate_count`, `eligible_candidate_count`, `suppressed_candidate_count`, and `applied_transform_count` so safe-mode observation counts meaningful candidates (not just applied transforms) without a second observe pass. `SafeModeObservation.to_summary_json()` always emits `source: "safe_apply"` and keeps `transform_counts` as applied-transform counts. Safe-mode `apply_safe_compression()` runs in a single pass: no-op returns the original payload object by identity, applied runs return a path-level copy-on-write payload that copies only the dict/list ancestors on each mutated path and shares unchanged subtrees by reference (this is *not* a full deep copy). `_copy_with_replacements()` is pinned by direct unit tests for: no-op identity, single dict path, single list path, multiple shared-prefix replacements, multiple disjoint branches, duplicate-path last-wins, invalid-path fail-closed, and chained transforms. The runtime dashboard renders a `Dispatch spans` panel that lists the actionable spans (`coordinator_pre_upstream_ms`, `segmentation_ms`, `compression_analyze_ms`, `compression_apply_ms`, `selection_lock_wait_ms`, `selection_locked_ms`, `routing_trace_write_ms`) with p50/p95/max plus `sample_count`, and absent spans render as "not observed in recent window" instead of `0 ms`.
- **Hot-path final polish**: request-path-level closure over Phases 1–5 + corrective polish. `tests/unit/test_proxy_request_hotpath_modes.py` exercises the full `handle_proxy_request()` pipeline (segmentation → safe apply / observe analyze / disabled short-circuit → `ProxyRequestContext` → coordinator dispatch) and pins each mode's helper calls and recorded spans. Safe mode calls `apply_safe_compression` exactly once and `analyze_compression` zero times; observe mode calls `analyze_compression` exactly once and `apply_safe_compression` zero times; disabled mode calls none of them and emits no compression spans. `tests/unit/test_runtime_dispatch_spans_dashboard.py` runs the JSON snapshot path through `RuntimeMetricsService` and the dashboard through `render_runtime()` end-to-end, asserting the `dispatch_spans` shape and the runtime panel's "not observed in recent window" empty state. Stale "in-place on a deep-copied payload" wording was removed from `src/eggpool/transcoder/compression/apply.py`; the corrected contract is the same path-level copy-on-write behavior already documented above. Verified locally: `uv run pytest tests/unit/test_proxy_request_hotpath_modes.py tests/unit/test_hotpath_corrective_polish.py tests/unit/test_runtime_dispatch_spans_dashboard.py -v`, `uv run pytest -m request_path -v`, `uv run pytest -m dashboard -v`, full `uv run pytest`, `uv run ruff check src tests scripts`, `uv run ruff format --check src tests scripts`, and `uv run pyright src scripts`.
