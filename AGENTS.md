...
- **Codex configsetup contract**: `eggpool configsetup codex` emits a non-secret HTTP/SSE Responses TOML block with `env_key = "EGGPOOL_API_KEY"`; it prints without `--print-secret`, never embeds the resolved key, and tells operators to use `eggpool getkey` to set the environment variable. Explicit `--model` is recommended; multiple catalog models must not trigger an invented choice.
- **Codex live qualification**: `scripts/smoke_codex_compat.sh` is opt-in and runs a fixed text phase plus a random-marker read-only shell-tool loop over HTTP/SSE Responses. Record the Codex CLI version and upstream surface/provider family without secrets; passing one run does not claim all future Codex versions, providers, or native server tools.
