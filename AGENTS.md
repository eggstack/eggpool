...
- **Live Configuration Rehash (Milestone A)**: `src/eggpool/config_validation.py` owns the reusable validation contract used by both `check-config` and `rehash`; `src/eggpool/config_reload_policy.py` owns the typed diff, reload policy, and `ReloadResult`/`ReloadStage`. See `plans/2026-07-13-live-config-rehash-milestone-a-validation-and-diff.md` and `architecture/README.md` § Live Configuration Rehash.
