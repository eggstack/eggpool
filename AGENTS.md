...
- **Negotiation-safe failure effects**: a bare/unknown 401 never disables credentials, advances health, or cascades; only explicit invalid/expired/revoked credential evidence disables the selected account. Deterministic wire auth/surface/schema rejection may reject only the selected wire candidate and enter one provider/model single-flight before downstream handoff. Leaders alone submit discovery candidates under the provider-wide abnormal-dispatch gate; followers share only the wire decision. 429/rate pressure ends discovery without candidate suppression, and cancellation cannot release unowned capacity. All account and wire retries consume one shared `1 + max_retries_before_stream` upstream-submission budget. See `architecture/deep-dive-retry.md` and `architecture/deep-dive-request-lifecycle.md`
- **Negotiation-safe failure effects**: a bare/unknown 401 never disables credentials, advances health, or cascades; only explicit invalid/expired/revoked credential evidence disables the selected account. Weak `model ... is not supported` evidence is wire-local only when the selected provider-scoped catalog knows the model, another declared surface exists, and the response is pre-handoff. Strong `model not found`/authoritative absence remains model quarantine/withdrawal and does not enumerate surfaces. Deterministic wire auth/surface/schema/model-on-surface rejection may reject only the selected wire candidate and enter one provider/model single-flight before downstream handoff. Leaders alone submit discovery candidates under the provider-wide abnormal-dispatch gate; followers share only the wire decision. 429/rate pressure ends discovery without candidate suppression, and cancellation cannot release unowned capacity. All account and wire retries consume one shared `1 + max_retries_before_stream` upstream-submission budget. See `architecture/deep-dive-retry.md` and `architecture/deep-dive-request-lifecycle.md`
