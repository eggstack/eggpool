...
- **Phase 9 synthetic cache controls are post-route and content-private**: `eggpool.transcoder.cache_synthesis` runs inside `RequestCoordinator._apply_synthetic_cache_controls` AFTER account selection and provider-bound transcoding (sees `context.upstream_protocol`, not `endpoint.protocol`). Selects protected `stable_prefix` segments from provider-bound segmentation and, when enabled and not in dry-run, mutates the provider-bound body with synthetic Anthropic-style `cache_control` annotations around stable-prefix containers. The selector is disabled by default and dry-run by default. Only `ttl = "ephemeral"` is currently accepted (`5m`/`1h` rejected at config load). Native `cache_control` is preserved (paths normalized to tuples internally). Structural-diff safety check rejects unexpected mutations and falls back to original. The `QuotaFairScorer` does NOT consume synthetic cache fields; routing stays load-based (request count + token count + active count + health). Migration `0045` adds 9 columns + 2 indexes to `requests`; `EXPECTED_SCHEMA_VERSION` in `scripts/check_database.py` is 45. Stats roll-up at `/api/stats/synthetic-cache-observability`. Per-policy overrides ride on Phase 6 `[[compression.policies]]` via the `synthetic_cache_*` overlay fields (now fired post-route for `match_provider_ids` / `match_provider_kinds`). `_overlay_config()` skips synthetic-cache fields so a policy row containing only synthetic-cache overrides does not trigger compression config validation warnings.
- **Phase 10 closed-loop threshold tuning is content-private and routing-isolated**: `eggpool.transcoder.compression.tuning` produces recommendations for the three tunable thresholds (`min_candidate_tokens`, `min_savings_tokens`, `max_compression_latency_ms`) only. The engine never inspects raw prompts, never enables `compress_static_prefix`, never adds/removes transforms, never toggles `mode`, and never modifies synthetic cache knobs. All suggested values are clamped to operator-defined `[compression.tuning.bounds]` ranges and capped per-step by `max_adjustment_pct`; a second recommendation within `cooldown_seconds` is suppressed with `REASON_COOLDOWN_ACTIVE`. Phase 10 is currently **recommendation-only**. Default mode `recommend` writes suggestions to `compression_tuning_recommendations` and the dashboard only; `mode = apply` is accepted at config time but does NOT currently register runtime overrides -- a future supervised background task must call `build_runtime_override()` then `registry.register()` before apply mode takes effect. `compute_recommendation` always tags recommendations as `recommendation_only` regardless of mode. The in-memory `RuntimeCompressionPolicyOverrideRegistry` and `apply_runtime_override` helper exist for forward compatibility but no production code path registers entries yet. Fail-closed: malformed entries are dropped without affecting the previous config. Migration `0046` adds `compression_tuning_recommendations` and `compression_tuning_overrides` audit tables; `EXPECTED_SCHEMA_VERSION` in `scripts/check_database.py` is 46. Stats roll-up at `/api/stats/compression-tuning`. The `QuotaFairScorer` does NOT consume tuning state; `runtime_metrics._snapshot_routing_runtime` guardrails now expose `routing_uses_compression_tuning: false`. Same-provider account fairness (e.g. multiple OpenAI subscriptions) is preserved because the registry never enters account scoring. Phase 10 deliberately does NOT implement cache-aware routing; that would requi
- **Phase 10/11 review pass** completed 2026-07-03: The structural-diff safety check now validates added `cache_control` paths against candidate container paths via `_validate_synthetic_cache_diff` (rejects cache_control on non-candidate containers, non-cache_control additions, removals, and changes). `resolve_selected_provider_kind` falls back from `catalog.providers` to `config.providers[provider_id].kind` for config-backed providers; `ProviderConfig.kind` field is now available. Streaming parity is verified: `_execute_streaming` calls `_apply_synthetic_cache_controls` post-route and persists `synthetic_cache_result` on all finalization paths. Native `cache_control` preservation is now covered across system blocks, tools, and message content blocks. See `plans/cache_compression_phase_10_11_review_pass.md`.re an explicit `routing.cache_aware = true` config flag plus per-provider support detection, a cost model using cached-token prices, backtesting, per-client opt-in, and dashboard warnings. See `plans/cache_compression_phase_10_closed_loop_threshold_tuning.md` and `architecture/README.md` § Closed-Loop Threshold Tuning (Phase 10).
