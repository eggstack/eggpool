...
- **Match evidence audit trail**: every non-exact accepted match writes a `model_info_match_evidence` row with `match_method`, `confidence`, and `diagnostics_json`. Inspect via `repo.list_match_evidence(model_id, source=...)` or `SELECT * FROM model_info_match_evidence`.
