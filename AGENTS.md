...
- **Request finalization**: `RequestFinalizationJob` keyed by `(proxy_request_id, attempt_id)`. `RequestFinalizationSupervisor` is the sole process-owned retry scheduler, using one bounded timer and capped retry age/backoff. Capacity rejects before ownership transfer, and exhausted jobs retire from active ownership. `DurableFinalizationResult` reports actual request/attempt/reservation terminal and transition facts; `FinalizationResult` separately tracks durable convergence and runtime cleanup.
