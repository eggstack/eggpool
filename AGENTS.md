...
- **Model routers are staged through Plan 164**: `[model_routers.<id>]` is optional, structurally validated, compiled during candidate generation construction, and live-reloadable as one mapping. The direct selector service supports exact route IDs, one repair, and default fallback; public virtual-model dispatch remains concrete until the later integration phase
