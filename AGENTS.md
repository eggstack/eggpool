...
- **DNS cache**: `OutboundClientManager` integrates a `DnsNetworkBackend` that caches resolved DNS entries in memory for the shared outbound client. The cache reduces connection latency for repeated requests to the same upstream hosts. Controlled by `[network.dns_cache]` config. Per-provider clients (`ProviderClientPool`) use their own connection pools and are unaffected.
