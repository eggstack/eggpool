...
- **Response isolation**: non-streaming response adaptation completes before durable `COMPLETED`; native invalid JSON remains pass-through when usage is optional, while malformed required transcoding is a truthful terminal error. Streaming sets an explicit `downstream_started` fact before delivery, closes upstream responses on every terminal path, and never catches cancellation as an ordinary failure.
...
- **Database recovery**: `DatabaseRecoveryController` — single-flight, bounded recovery keeps public reads/writes closed until candidate schema verification, private writable probing, and ambiguity reconciliation succeed; `Database.admit_recovered_connection()` publishes admission and `READY` together; unresolved work is retained and buffer overflow fails closed.
