...
- Plans: `plans/` is append-only history (~200 files, mostly closed) plus the post-251 hierarchy (`plans/README.md`, `plans/registry.md`). See the `plan` skill before adding one. `.agents/` holds no custom agent definitions.
