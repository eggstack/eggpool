...
- **Routing-decision score components** (`src/eggpool/routing/router.py::RoutingDecisionTrace.score_components`) carry the per-account score breakdown on every persisted `routing_decisions` row (column `score_components_json` added by migration `0035`). The dashboard and `eggpool accounts explain` subcommand consume this directly; do not re-score from quota tables when only the diagnostic breakdown is needed. The payload now also includes per-window `util_5h` / `util_7d` / `util_30d` utilization ratios (None when capacity is unconfigured) and a `tie_break` summary naming the decisive factor between the chosen account and its runner-up (`tier`, `quota`, `inflight`, `transcode`, `near_tie`, `exact_tie`, `no_runner_up`).
- **`_select_lock` publish ordering**: in `RequestCoordinator._select_and_persist_attempt()`, the runtime publication step (`Router.increment_active_request_count` + `QuotaEstimator.add_reservation`) must run INSIDE `_select_lock` AFTER the durable transaction commits but BEFORE the lock releases. The two contexts are explicit nested `async with` blocks (outer `_select_lock`, inner `_db.transaction()`). Do not collapse this back into a compound context: a compound `async with self._select_lock, self._db.transaction():` exits right-to-left, so the transaction would still commit before the lock released — that is not the bug. The actual bug was that the runtime publication block lived INSIDE the transaction body, so active-count and reserved-cost state were published before the transaction committed. The explicit nested form makes it hard to accidentally place publication inside the transaction while still keeping publication under `_select_lock`. The key invariant is block placement (publication must be outside the DB transaction body but still inside `_select_lock`), not context-exit order. Keep the compensation chain (`decrement` → `finalize_failed_attempt(asyncio.shield(...))` → `health_manager.release_request` → set `client_metadata["post_commit_interrupted"]` → `raise`) wrapping the publish step; the outer `except BaseException:` catches `CancelledError` / `SystemExit` / `KeyboardInterrupt` and re-raises them after compensation so they cannot be swallowed.
- **`eggpool accounts explain` reads from SQLite, not an empty cache**: the command opens the database, runs migrations on a fresh install, and calls `ModelCatalogCache.hydrate_from_db(db)` to populate the in-memory model/provider/account-support tables from `models`, `provider_model_metadata`, and `account_models` rows. A thin `_CatalogShim` exposes the loaded cache as a `CatalogService`-compatible object so `Router.explain_account_eligibility` can be reused unchanged without booting a full `CatalogService`. Output is rendered with `click.echo` (no `rich` dependency). `reason_detail` strings now include the account name, provider id, configured protocols, requested model id, and stale-window seconds so the operator can act directly on the diagnosis.
...
- **DNS cache**: `OutboundClientManager` and `ProviderClientPool` both integrate a `DnsNetworkBackend` that caches resolved DNS entries in memory. The cache reduces connection latency for repeated requests to the same upstream hosts. Controlled by `[network.dns_cache]` (enabled by default, TTL 1800s, max 50 entries). When a proxy is configured for an account, that account's client uses the proxy transport instead of the cached backend. Exposes precise counters (`cache_hits_total`, `cache_misses_owner_total`, `singleflight_waits_total`, `resolver_calls_total`) and derived rates (`dns_suppression_rate`, `cache_hit_rate`) for operator diagnostics.
- **Memory footprint caps**: every growth axis in `QuotaEstimator`, `ModelCatalogCache`, `CatalogResolverPipeline.TTLCache`, `OutboundClientManager`, `AccountRuntimeState`, and `HealthManager` is now bounded by a hardcoded cap (`EWMA_HARD_CAP = 4096`, `GLOBAL_EWMA_HARD_CAP = 1024`, `MAX_TRACKED_HOSTS = 256`) or per-catalog `max_entries` (default 4096). The regression gate is `tests/integration/test_memory.py` (`pytest.mark.slow`). See `plans/memory.md` for the full design.
