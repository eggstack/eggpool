...
- **Field reload classification lives in `src/eggpool/config_reload_policy.py`.** Every `AppConfig` field defaults to `RESTART_REQUIRED`; the policy map (`_FIELD_DISPOSITION`) is the single reviewable inventory. The closure pass classifies provider/account/routing/model-override families as `LIVE`. Every other field stays `RESTART_REQUIRED` until a live replacement path is added in a separate diff that moves the entry to `LIVE` at the same time. The `_disposition_for()` helper also inherits LIVE from `providers`, `accounts`, `model_overrides`, and `model_capabilities` for the expanded per-key paths (`providers.<id>`, `accounts.<provider>/<name>`, etc.) so adding a new provider through `rehash` publishes a generation rather than rejecting with `restart_required`. The unit test `test_live_field_inventory_matches_expected` pins the actual set of LIVE paths; adding a field that silently slips past this test is a regression.
