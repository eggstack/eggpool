...
- **Model capabilities**: protocol-neutral `ThinkingCapability` / `ModelCapabilities` schema in `src/eggpool/catalog/capabilities.py` with deterministic merge (defaults → provider catalog → global overrides → provider-scoped overrides), aggregate semantics (all-supported/all-unsupported/mixed/conflicting), compact serialization for `/v1/models`, and request-level helpers (`client_requests_thinking()`, `has_thinking_support()`). Protocol compatibility alone does not imply thinking support. See `architecture/README.md` § Model Capabilities.
