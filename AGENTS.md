...
- **Process model**: supervisor + Granian worker (`workers=1`), daemon mode (`--verbose` for foreground). `runtime_threads=1` is required; other values fail configuration validation. `database_worker_threads=2`. Readiness probe is process-owned.
- **Runtime generations**: `RuntimeManager` owns active/retiring generation slots. Lease acquisition is fail-closed: `RuntimeManagerLeaseExhaustedError` → HTTP 503. Staged reload swap: `stage()` → `commit()`/`rollback()` → `finalize_retirement()`. `RuntimeGenerationCandidate` owns reload-created resources; `candidate.abort()` closes in reverse order. `RequestFinalizationSupervisor` is generation-owned: each accepted retained job holds one terminal retirement reference until durable and required runtime convergence; retirement closes only after both request leases and terminal references reach zero. A live unresolved reference triggers the existing fatal worker handler; bounded process shutdown may abandon references for startup repair.
