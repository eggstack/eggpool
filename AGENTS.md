...
- **Model information**: `model_info/` sidecar subsystem with persistent metadata, provider-native observations, status classification, lifecycle wiring (`ModelInfoService`), background refresh scheduler (`ModelInfoRefreshScheduler`), catalog-refresh-driven reconciliation (`CatalogRefreshResult`), and CLI inspection. See `architecture/README.md` § Model Information.
