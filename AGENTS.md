...
# Plan 029 — Dispatch writer and observability bounds (bounded sample
# storage, precise metric semantics, request-coherent span sampling,
# metric cardinality, backpressure diagnostics)
uv run pytest \
    tests/unit/test_plan_029_span_sampling.py \
    tests/unit/test_plan_029_dispatch_writer_metrics.py \
    tests/unit/test_plan_029_metric_cardinality.py -v
...
- **Plan 027 — Database Connection Recovery and Transaction Reconciliation**: allows EggPool to recover safely from an invalidated or indeterminate SQLite connection without requiring a process restart. Process-owned recovery controller replaces a suspect connection, reconciles ambiguous operations, and restores readiness. Key changes: (1) `DatabaseLifecycleState` enum (`src/eggpool/db/connection.py`) — explicit state machine (`disconnected → connecting → ready → invalidating → invalidated → recovering → reconciling → ready / failed_closed → shutting_down`). (2) `connection_epoch` property — incremented on every successful `connect()` so long-lived components detect replacement. (3) `DatabaseRecoveryController` (`src/eggpool/db/recovery.py`) — single-flight recovery with bounded retry/escalation, reason-class tracking, and `RecoverySnapshot` diagnostics. (4) `AmbiguousDatabaseOperation` — frozen dataclass capturing indeterminate commit metadata for dispatch/finalization reconciliation. (5) `DatabaseRollbackError` — typed error when ROLLBACK itself fails after a body exception, distinct from `DatabaseCommitError`. (6) `_safe_rollback()` helper with bounded diagnostics. (7) `[database.recovery]` config section with `max_attempts`, `initial_backoff_ms`, `max_backoff_ms`, `reconciliation_timeout_s`, `fail_process_on_exhaustion`. (8) Wired into `ProcessRuntime.recovery_controller`, app.py startup/shutdown, `/readyz` recovery-state degradation. (9) `WritableProbe.force_probe_nowait()` for recovery-cycle refresh. Tests: `tests/unit/test_plan_027_database_lifecycle.py`, `tests/unit/test_plan_027_recovery_singleflight.py`.
- **Plan 028 — Provider Payload Lifecycle and Hot-Path Consolidation**: reduces request-path CPU, allocations, serialization work, and SQLite writer-lock duration by consolidating provider-bound request transformations around one decoded payload lifecycle, consolidating non-stream response processing around one decoded response lifecycle, and moving avoidable lookups and best-effort work outside correctness-critical transactions. Key changes: (1) `ProviderBoundRequest` (`src/eggpool/request/provider_bound_request.py`) — typed lifecycle object owned by one proxy request; carries `client_payload` (immutable), `provider_payload` (copy-on-write via `set_provider_payload`), `provider_bytes` (serialized once), `payload_generation` counter, and `SegmentationValidityKey` for cache reuse. (2) `SegmentationValidityKey` / `PreparedTranscodeValidityKey` (`src/eggpool/request/provider_bound_request.py`) — frozen dataclass compound keys that make segmentation and transcode reuse deterministic. (3) `TransformPipeline` (`src/eggpool/request/transform_pipeline.py`) — ordered post-selection transform pipeline with declarative `TransformMeta` (requires_decoded_payload, invalidates_segmentation, changes_token_estimates, may_fail_request, diagnostic_category), `TransformResult` (decision + warnings), and `run_transform_pipeline()` orchestrator that short-circuits on rejection. (4) `ParsedUpstreamResponse` (`src/eggpool/request/parsed_upstream_response.py`) — single-decode lifecycle for non-streaming upstream responses; lazily parses body once and provides `parsed_dict`, `parse_status`, and `header_value()` accessors so usage extraction, normalized usage construction, and response transcoding share one decoded representation. (5) `ProxyRequestContext.provider_bound` field — attaches the `ProviderBoundRequest` to the request context so `body_for_upstream` delegates to `provider_bound.provider_bytes`. (6) `RequestCoordinator._extract_non_stream_usage_from_parsed()` — reads from `ParsedUpstreamResponse.parsed_dict` instead of re-parsing bytes, eliminating duplicate JSON decode in the non-streaming success path. (7) `RequestFinalizer._precompute_finalization_diagnostics()` — precomputes all diagnostic serialization (segmentation summary JSON, compression observation/result JSON, synthetic cache JSON, resolved policy JSON) outside the `BEGIN IMMEDIATE` transaction, and moves best-effort account event enrichment post-commit, shortening the SQLite write-lock duration. (8) `ProviderBoundRequest` created in `handle_proxy_request()` and attached to `ProxyRequestContext` before coordinator dispatch. Tests: `tests/unit/test_plan_028_provider_bound_request.py`, `tests/unit/test_plan_028_transform_pipeline.py`, `tests/unit/test_plan_028_parsed_upstream_response.py`.

- **Plan 029 — Dispatch Writer and Observability Bounds**: makes dispatch persistence diagnostics, detailed request instrumentation, and batching behavior safe for indefinite process lifetime. Key changes: (1) **Bounded sample storage** — all dispatch-writer sample deques use `deque(maxlen=sample_window)`; snapshot p95 remains stable after 1M synthetic batches. (2) **Precise metric semantics** — `enqueue_wait_ms` (queue-capacity wait), `queue_age_ms` (enqueue→claim), `batch_formation_wait_ms` (first intent→batch close), `transaction_ms` (one per batch), `result_delivery_ms` (commit→futures signaled), `intent_end_to_end_ms` (submission→result delivery), `batch_size` (one per batch). (3) **One-sample-per-event accounting** — transaction duration and batch size sampled once per batch, not per result; failed batches and failed intents counted separately; cancellation stage counters (before claim / after claim / after commit) are exact. (4) **Request-coherent span sampling** — `DispatchSpanRecorder.should_sample_request()` uses a deterministic SHA-256 hash of the request ID for a stable per-request decision; the decision is propagated via a `ContextVar` so the coordinator's shared recorder respects it; coarse metrics remain always-on. Production default is 5% (was 100%). (5) **Config** — `[metrics.dispatch_spans]` section with `sample_rate` and `window_size`; legacy `detailed_span_sample_rate` deprecated but overrides. (6) **Backpressure diagnostics** — saturation count, submit timeout count, occupancy ratio, oldest intent age. Tests: `tests/unit/test_plan_029_span_sampling.py`, `tests/unit/test_plan_029_dispatch_writer_metrics.py`, `tests/unit/test_plan_029_metric_cardinality.py`.
...
- `DatabaseCommitError` has `rollback_attempted`, `rollback_succeeded`, `transaction_still_active`, `connection_invalidated`, `outcome`
- `DatabaseRollbackError` has `rollback_attempted`, `rollback_succeeded`, `transaction_still_active`, `connection_invalidated`, `original_exception`
