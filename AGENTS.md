...
- **Field reload classification lives in `src/eggpool/config_reload_policy.py`.** Every `AppConfig` field defaults to `RESTART_REQUIRED`; the policy map (`_FIELD_DISPOSITION`) is the single reviewable inventory. Milestone C's live-reload infrastructure is complete but no field is classified `LIVE` yet — when a future milestone adds a live-reload path for a field, move the corresponding entry to `LIVE` in the same diff. Never mark a field `LIVE` until the live replacement path exists.
