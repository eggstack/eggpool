...
- **Native transcode capabilities are explicit**: `ModelCapabilities.transcoding` lists verified target protocols for structured output, strict tools, parallel-tool control, prompt-cache breakpoints, and reasoning efforts. Protocol strings alone never authorize native fields; missing facts are unknown. Prompt-cache TTLs are never silently converted, tool-definition boundaries are never moved to message boundaries, and cache keys are never synthesized or logged.
- **Compression/cache surface**: compression tuning is recommendation-only (`off` or `recommend`); recommendations never mutate live policy. Static-prefix override legality is checked after global policy and child override resolution. Explicit native cache boundaries are preserved and suppress synthetic insertion at the same provider-bound placement.
...
- **Prepared transcode ownership**: cached immutable mapping/tuple payloads are materialized into one ordinary provider graph at the `ProviderBoundRequest` boundary; do not pass mapping proxies to `deepcopy`.
