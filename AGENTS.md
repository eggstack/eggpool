...
- **Retained terminal commands**: one tagged coordinator-owned registry stores failed-attempt cleanup and post-commit claim compensation. The single shutdown drain dispatches by stored command kind, counts unique durable identities, and leaves incomplete progress registered for rejoin or startup crash repair. A command kind cannot change for an existing `(proxy_request_id, attempt_id)`.
