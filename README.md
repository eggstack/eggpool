...
- Deployment-suffix matching (tier 2b, opt-in via `matching.deployment_suffix_normalized_exact = true`) strips deployment-shaped tokens like `highspeed`, `fast`, `turbo`, `preview`, `experimental`, `nitro`, `max`, `beta`, `default`, and `base` only when the original identifier carries a digit/family anchor, the candidate set is unique, and the original contains no semantic-variant token (`pro`, `mini`, `flash`, `lite`, `nano`, `instruct`, `chat`, `reasoning`, `thinking`, `haiku`, `sonnet`, `opus`, `coder`, `codex`). So `MiniMax-M2.7-highspeed` resolves to `minimax/minimax-m2.7` while `claude-3-haiku-highspeed` keeps its semantic shape. Artificial Analysis matching shares the same tiered resolver and the same match-evidence audit trail as OpenRouter.
