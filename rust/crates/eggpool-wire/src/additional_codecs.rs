...
use crate::adaptation::{client_wire_surface, notice, request_notices, stable_tool_call_id};
use crate::codec::{
    CodecError, CodecOutput, CodecReasonCode, DecodedProviderPayload, StreamAdapterKind, WireCodec,
    WireCodecId,
};
use crate::codecs::{encode_anthropic_response, encode_openai_response};
use crate::decode::{DecodeError, DecodeLimits, canonical_request_from_value_with_limits};
use crate::ir::{
...
use crate::profile::{CodecFamily, ConfiguredWireProfile, WireSurface};
        ...
        match item.get("type").and_then(Value::as_str) {
            ...
            Some("tool_search_call") => {
                ...
                output.push(CanonicalOutputBlock {
                    ...
                    media: None,
                    call_id,
                    name: Some(crate::ir::TOOL_SEARCH_TOOL_NAME.to_owned()),
                    ...
                    match block.get("type").and_then(Value::as_str) {
                        ...
                        Some("output_image") | Some("output_file") => {
                            ...
                            output.push(CanonicalOutputBlock {
                                ...
                                text: None,
                                media: Some(crate::codecs::decode_response_media(
                                    block,
