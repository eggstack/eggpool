...
- **Prepared transcode ownership**: preflight retains one request-local translated generation without recursive physical freezing. Valid unchanged reuse adopts it through `adopt_provider_payload()` and reuses its encoded bytes; later provider-specific changes establish COW or conservative ownership before mutation. Prepared graphs never cross request boundaries.
- **Provider payload memory**: canonical parsed state is logically immutable. Narrow provider changes use path-level copy-on-write, trusted EggPool-owned graphs use `adopt_provider_payload()`, unknown graphs use conservative deep ownership, native/no-transform dispatch reuses accepted client bytes, and stream handoff releases dispatch-only buffers after retry is impossible.
