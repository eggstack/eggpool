...
- **Historical repair workflow**: `eggpool stats repair-costs [--provider P] [--since YYYY-MM-DD] [--dry-run|--apply]` is the targeted escape hatch for suspicious historical rows (near-cap totals, implausible cost/token, estimated rows far above reservation). Apply mode updates both `cost_microdollars` and `exactness`, skips provider-reported rows, and writes an audit row to `request_cost_repairs`. `eggpool stats recompute-costs` remains the broader full-table recalculation tool
