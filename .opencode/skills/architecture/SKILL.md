...
- **Frozen prepared payloads**: cached transcode mappings/tuples are materialized into one ordinary provider graph at the ownership boundary; immutable mapping proxies are never passed to `deepcopy`.
- **Provider payload memory**: canonical parsed state is logically immutable; the first provider mutation owns one deep ordinary-graph copy, native/no-transform dispatch reuses accepted client bytes, and stream handoff releases dispatch-only buffers after retry is impossible.
