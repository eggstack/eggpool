...
- **Cache compression**: Phase 4 observe-mode accounting lives in `src/eggpool/transcoder/compression/` (`analyze_compression` returns a `CompressionObservation` per request; persisted via migration 0042); Phase 5 adds a safe-mode mutating applier (`apply_safe_compression` returns a `CompressionResult`, persists via migration 0043, fails-closed on stable-prefix content hash mismatch). Segmentation `content_path` values are concrete JSON paths resolving to actual string leaves; `resolve_path` and `resolve_text_path` helpers are available. `stable_prefix_content_hash` is an exact SHA-256 re-extracted from both original and transformed payloads. All six transforms emit unified markers via `markers.build_marker`. Context-limit checks happen before compression. The Phase 5 corrective pass (2026-07-02) fixed production path resolution, exact content hashing, and fail-closed verification to actually re-hash the mutated payload. Controlled by `[compression]` config; observe mode default.
