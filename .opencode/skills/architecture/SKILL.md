...
- **Cache read share is bounded**: `cache_read / (input + cache_read + cache_write)`, returns `None` when the denominator is zero. Same denominator for `cache_write_ratio`. The unbounded `cache_read / input` formula is retired across `_build_summary`, `fetch_account_stats`, `fetch_model_stats`, the rollup-backed `get_summary_from_rollups`, and the dashboard "Cache tokens" card
- **Summary payload adds `fresh_tokens` (= input + output) and `accounted_tokens` (= input + output + cache_read + cache_write)** alongside the existing `total_tokens`. `total_tokens` keeps its input+output meaning for backward compatibility
