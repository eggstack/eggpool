...
- `[server].threads` (int, default `2`, min `1`, max `64`) controls Granian `runtime_threads` (the number of worker event-loop threads). Default `2` keeps dashboard responsiveness reasonable on Raspberry Pi 4/5 while a single event-loop thread multiplexes streaming proxy traffic, dashboard requests, and lightweight background work. `1` is the documented minimum-footprint override for extremely constrained devices; raise on capable hardware for higher concurrency. Granian `workers=1` is fixed and not configurable in this release.
...
- **Cache read share is bounded**: `cache_read / (input + cache_read + cache_write)`, returns `None` when the denominator is zero. Same denominator for `cache_write_ratio`. The unbounded `cache_read / input` formula is retired across `_build_summary`, `fetch_account_stats`, `fetch_model_stats`, the rollup-backed `get_summary_from_rollups`, and the dashboard "Cache reads" card
...
- **Route registration order**: `register_model_info_routes()` in `src/eggpool/api/model_info.py` registers specific suffix routes (`/matches`, `/aliases`) BEFORE the greedy `/{model_id:path}` detail route. Without this ordering FastAPI's path matcher captures `<model_id>/aliases` as the `model_id` and dispatches to the detail handler. `tests/unit/test_model_info_route_registration.py` pins both the order and a functional TestClient dispatch test
...
- **Tiered identity matching** (`src/eggpool/model_info/matching.py`): resolves local model IDs to source records through 5 tiers — `configured_exact_alias` (operator aliases), `exact_source_id` (verbatim source id), `normalized_exact` (NFKC/casefold/separator-strip normalization), `regex_rule` (built-in patterns for `minimax`, `claude`, `gemini`), `similarity_guarded` (SequenceMatcher, disabled by default). Provider namespaces like `opencode-go` are stripped via `strip_provider_namespace` before matching. Non-exact accepted matches persist evidence rows in `model_info_match_evidence` and stamp `match_method`/`discovered_by`/`diagnostics_json` on `model_info_aliases`.
