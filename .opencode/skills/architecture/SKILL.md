...
- **Cache compression**: Phase 4 observe-mode accounting lives in `src/eggpool/transcoder/compression/` (`analyze_compression` returns a `CompressionObservation` per request; persisted via migration 0042); Phase 5 adds a safe-mode mutating applier (`apply_safe_compression` returns a `CompressionResult`, persists via migration 0043, fails-closed on stable-prefix content hash mismatch). Segmentation `content_path` values are concrete JSON paths resolving to actual string leaves; `resolve_path` and `resolve_text_path` helpers are available. `stable_prefix_content_hash` is an exact SHA-256 re-extracted from both original and transformed payloads. All six transforms emit unified markers via `markers.build_marker`. Context-limit checks happen before compression. The Phase 5 corrective pass (2026-07-02) fixed production path resolution, exact content hashing, and fail-closed verification to actually re-hash the mutated payload. Phase 6 adds per-request policy overrides (`[[compression.policies]]`) via `resolve_compression_policy` in `policy_resolver.py`; pre-route matchers are `match_clients`, `match_protocols`, `match_requested_models`, `match_transcoded`; provider-specific matchers are reserved for post-route. Match semantics union OR, last-match-wins for scalars, field-by-field merge for `transforms`. Fail-closed on malformed overrides; static-prefix guard preserved. Persisted via migration 0044. The `QuotaFairScorer` does NOT consume compression or policy fields; routing stays load-based. Controlled by `[compression]` config; observe mode default. **Phase 7 (dashboard/runtime views)** makes the data from Phases 1–6 operationally usable: six JSON endpoints under `/api/stats/` (`cache-observability`, `canonical-request-segmentation`, `cache-stability`, `compression-observability`, `compression-runtime`, `compression-policies`) and four new runtime cards on `/runtime` (compression, compression runtime, compression policy, cache stability) with a static routing-separation notice that cache/compression metrics are reporting-only and not consumed by the `QuotaFairScorer`. No raw prompts, tool outputs, system messages, request bodies, or auth headers appear in any card or JSON response. No new migrations. See `plans/cache_compression_phase_07_dashboard_runtime_views.md` and `architecture/README.md` § Dashboard and Runtime Views (Phase 7).
- **Routing guardrails (Phase 8)**: codifies the routing invariant that cache/compression metrics NEVER enter account scoring, health removal, or route reselection. Three pins: (1) **hardcoded runtime diagnostic** — `RuntimeMetricsService._snapshot_routing_runtime` returns a constant `guardrails` dict (`routing_cache_compression_mode: "reporting_only"`, `routing_uses_cache_metrics: false`, `routing_uses_compression_metrics: false`, `routing_uses_stable_prefix_hash: false`, `routing_uses_compression_policy: false`, plus the allowed scorer input list); exposed via `GET /api/stats/runtime` and rendered as a **Routing guardrails (Phase 8)** card on the runtime page next to the routing-separation notice. (2) **Static + behavioural test pin** — `tests/unit/test_routing_guardrails.py` (19 tests) asserts the `QuotaFairScorer.score_accounts` signature accepts only the four canonical inputs (`account_names`, `model_name`, `active_requests`, `request_estimates`), that `RoutingScore` carries no cache/compression field, that identical load with adversarial cache/compression metrics produces identical scores, that two same-provider accounts with skewed cache hits / compression savings / stable-prefix hashes still get fair rotation, that compression fallbacks (`apply_safe_compression`'s `failed_fallback=True`) never affect provider health, that policy resolution (`resolve_compression_policy`) is observational-only and never reroutes. (3) **Documentation invariant** — every phase doc states the scorer does NOT consume the phase's columns. Same-provider account fairness (e.g., multiple OpenAI subscriptions) is preserved because cache hit ratios or compression savings never enter the score. A future cache-aware routing mode would require an explicit `routing.cache_aware = true` config flag plus per-provider support detection, a cost model using cached-token prices, backtesting, per-client opt-in, and dashboard warnings; Phase 8 deliberately does NOT implement it. No new migrations. See `plans/cache_compression_phase_08_routing_guardrails.md` and `architecture/README.md` § Routing Guardrails and Non-Interference (Phase 8).


- **Closed-loop threshold tuning (Phase 10)**: `eggpool.transcoder.compression.tuning` produces advisory recommendations for the three tunable thresholds (`min_candidate_tokens`, `min_savings_tokens`, `max_compression_latency_ms`). The engine is content-private (no prompt inspection), bounded (every suggested value clamped to `[compression.tuning.bounds]`), rate-limited (`max_adjustment_pct` per step; `cooldown_seconds` suppresses the next recommendation), and immutable on every other compression knob (mode, enabled, static-prefix, transforms, synthetic cache knobs). `mode = "recommend"` (default) writes to the `compression_tuning_recommendations` table and dashboard only. `mode = "apply"` is accepted at config time but does NOT currently register runtime overrides -- a future supervised background task must call `build_runtime_override()` then `registry.register()` before apply mode takes effect. The in-memory `RuntimeCompressionPolicyOverrideRegistry` and `apply_runtime_override` helper exist for forward compatibility but no production code path registers entries yet. Migration `0046_closed_loop_threshold_tuning.sql` adds the audit tables; `EXPECTED_SCHEMA_VERSION` is 46. Stats roll-up at `/api/stats/compression-tuning`. Runtime metrics now expose `routing_uses_compression_tuning: false`. The `QuotaFairScorer` does NOT consume tuning state. Phase 10 deliberately does NOT implement cache-aware routing. See `plans/cache_compression_phase_10_closed_loop_threshold_tuning.md` and `architecture/README.md` § Closed-Loop Threshold Tuning (Phase 10).
- **Post-route synthetic cache controls (Phase 9)**: `eggpool.transcoder.cache_synthesis` runs inside `RequestCoordinator._apply_synthetic_cache_controls` AFTER account selection and provider-bound transcoding (sees `context.upstream_protocol`, not `endpoint.protocol`). The selector operates on provider-bound segmentation and only annotates protected `stable_prefix` containers from `SYSTEM`/`DEVELOPER`/`TOOL_SCHEMA` sources. Path representation is normalized to `tuple[str | int, ...]` internally; native `cache_control` is preserved byte-for-byte. Apply mode validates structural diff (only `cache_control` additions allowed at candidate containers) and falls back to original payload on unexpected changes. Only `ttl = "ephemeral"` is currently accepted; `5m`/`1h` rejected at config load. Disabled by default and dry-run by default. Per-policy overrides ride on Phase 6 `[[compression.policies]]` via `synthetic_cache_*` fields (post-route resolver fires `match_provider_ids` / `match_provider_kinds`). Structural-diff safety now uses `_validate_synthetic_cache_diff` which validates added `cache_control` paths against the candidate container paths, not merely the suffix. `resolve_selected_provider_kind` consults `config.providers[provider_id].kind` as a fallback for config-backed providers. Migration `0045` adds 9 columns + 2 indexes to `requests`; `EXPECTED_SCHEMA_VERSION` is 45. The `QuotaFairScorer` does NOT consume synthetic cache fields; routing stays load-based. See `plans/cache_compression_phase_09_synthetic_cache_controls.md`.
