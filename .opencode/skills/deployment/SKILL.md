...
`scripts/install.sh` installs `eggpool` from the package channel through a
single selected manager (existing uv/pipx/pip ownership is adopted; fresh
installs prefer uv tool). It does not clone the repository or require a
Python application runtime. It refuses ambiguous ownership, manager/PATH
collisions, root personal installs, and standalone-binary replacement unless
`--adopt-standalone` is explicit. Use `--version X.Y.Z` (or `vX.Y.Z`),
`--upgrade`, or `--force` for exact, latest, or repair transitions. It seeds
the XDG config through the installed package without overwriting existing
config or `.env`.
...
- See `docs/deployment.md` Performance Profiles for the lean default and full-diagnostics profile.
