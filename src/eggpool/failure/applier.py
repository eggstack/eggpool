...
class EffectsApplier:
    ...
    def _apply_account_effect(
        ...
        if effects.account_effect == "disable_auth":
            ...
            # Per-model failures (``model_effect = "quarantine"``) must
            # never advance the account-wide circuit breaker: a single
            # model/provider anomaly should not black-hole sibling
            # models on the same account.  The per-model disable applied
            # via ``_apply_model_effect`` is the only shared-state
            # penalty in that case.  Only when the failure is genuinely
            # account-wide (no per-model suppression requested) do we
            # call ``record_failure`` to advance the breaker counter.
