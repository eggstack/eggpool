"""Focused contract tests for the release packaging Rust wheel publication boundary."""
...
from scripts.inspect_release_wheel import WheelInspectionError, inspect_wheel

ROOT = Path(__file__).parents[2]
PUBLICATION_MANIFEST = ROOT / "packaging/pypi/pyproject.toml"
WHEEL_DIR = ROOT / "dist/native_release"
...
def test_root_project_is_tooling_only_and_not_publishable() -> None:
    ...
    assert root_manifest["tool"]["eggpool"] == {
        "project_role": "repository-tooling-only",
...
def test_built_wheel_obeys_the_contract_when_available() -> None:
    ...
    if not wheels:
        pytest.skip("run the release packaging build before artifact-level inspection")
