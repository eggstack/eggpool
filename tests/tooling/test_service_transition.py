"""Contract tests for the guarded service transition deployed-transition runner."""
...
from scripts.qualification_service_transition import (
    bounded,
...
def test_runner_requires_disposable_host_acknowledgement(tmp_path: Path) -> None:
...
def test_runner_blocks_without_linux_mutation(tmp_path: Path) -> None:
...
def test_unit_uses_one_stable_executable_and_redacts_output(
    tmp_path: Path,
) -> None:
...
def test_personal_cleanup_disables_linger_before_user_delete() -> None:
    ...
    cleanup_personal_user(runner, "service-transition-test")  # type: ignore[arg-type]
    ...
    assert runner.calls[-1][1] == ["userdel", "-r", "service-transition-test"]
