...
async def _populate(repo: AccountBackoffRepository) -> None:
...
@pytest.mark.asyncio()
async def test_endpoint_returns_empty_list_when_no_backoffs(
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get("/api/backoffs")
...
@pytest.mark.asyncio()
async def test_endpoint_returns_active_backoffs(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
...
@pytest.mark.asyncio()
async def test_endpoint_omits_expired_backoffs(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
    app, repo = app_with_repo
    await repo.upsert_failure(
...
@pytest.mark.asyncio()
async def test_endpoint_honors_now_query_parameter(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
    app, repo = app_with_repo
    ...
    async with httpx.AsyncClient(
        ...
        active_response = await client.get(f"/api/backoffs?now={cutoff}")
        expired_response = await client.get(f"/api/backoffs?now={cutoff + 120}")
...
@pytest.mark.asyncio()
async def test_endpoint_rejects_invalid_now_query_parameter(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
    app, _repo = app_with_repo
    async with httpx.AsyncClient(
        ...
        response = await client.get("/api/backoffs?now=not-a-timestamp")
    assert response.status_code == 400
    assert response.json() == {"error": "now must be a POSIX epoch timestamp"}


@pytest.mark.asyncio()
async def test_endpoint_rejects_non_finite_now_query_parameter(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
    app, _repo = app_with_repo
    async with httpx.AsyncClient(
        ...
        response = await client.get("/api/backoffs?now=inf")
    assert response.status_code == 400
    assert response.json() == {"error": "now must be a finite POSIX epoch timestamp"}


@pytest.mark.asyncio()
async def test_endpoint_includes_iso_backoff_until(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
    app, repo = app_with_repo
    await _populate(repo)
...
@pytest.mark.asyncio()
async def test_endpoint_reports_account_name_lookup_failure(
    app_with_repo: tuple[FastAPI, AccountBackoffRepository],
) -> None:
...
@pytest.mark.asyncio()
async def test_endpoint_returns_503_when_repo_missing() -> None:
    app = FastAPI()
    app.state.db = None
    register_backoff_routes(app)
    async with httpx.AsyncClient(
