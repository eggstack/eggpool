...
class TestDispatchWriterParity:
    ...
    async def test_dispatch_writer_wired_to_coordinator(self) -> None:
        ...
        try:
            ...
            config = _make_config()
            config.dispatch_writer.enabled = True
            ...
            # And writer selection should be enabled
            assert result.coordinator._use_dispatch_writer is True
        finally:
            await db.disconnect()

    @pytest.mark.asyncio()
    async def test_writer_enabled_selects_writer_path(self) -> None:
        """When writer is present and config enabled, coordinator uses writer path."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
    ...
    @pytest.mark.asyncio()
    async def test_writer_disabled_no_writer_object_uses_direct_path(self) -> None:
        """When writer is None, coordinator falls back to direct persistence."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
        ...
        try:
            await MigrationRunner(db).run()
            process = _make_process(db=db, stats_db=db)
            process.dispatch_writer = None
            config = _make_config()
            config.dispatch_writer.enabled = False
            ...
            assert result.coordinator._use_dispatch_writer is False
            assert result.coordinator._dispatch_writer is None
        finally:
            await db.disconnect()

    @pytest.mark.asyncio()
    async def test_writer_present_but_config_disabled_uses_direct_path(self) -> None:
        """When writer exists but config disabled, coordinator uses direct path."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
    ...
    @pytest.mark.asyncio()
    async def test_writer_config_enabled_but_no_writer_object_uses_direct_path(
        self,
    ) -> None:
        """When config enabled but writer is None, coordinator uses direct path."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
    ...
    @pytest.mark.asyncio()
    async def test_writer_selection_survives_reload(self) -> None:
        """Writer selection flag persists across factory reload calls."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
        ...
        try:
            ...
            # Both should have writer selection enabled
            assert startup_result.coordinator._use_dispatch_writer is True
            assert reload_result.coordinator._use_dispatch_writer is True

            # Both should share the same process-owned writer
            assert startup_result.coordinator._dispatch_writer is dispatch_writer
            assert reload_result.coordinator._dispatch_writer is dispatch_writer

            # Writer identity is process-owned and stable
            assert (
                startup_result.coordinator._dispatch_writer
                is reload_result.coordinator._dispatch_writer
            )
        finally:
            await db.disconnect()

    @pytest.mark.asyncio()
    async def test_writer_not_closed_on_generation_retirement(self) -> None:
        """Generation retirement does not close the process-owned writer."""
        from eggpool.db.connection import Database
        from eggpool.db.migrations import MigrationRunner
        ...
        try:
            ...
            # The writer should not be in the candidate resources
            # (it is process-owned, not generation-owned)
            candidate = RuntimeGenerationCandidate(generation_id=1)
            factory2 = RuntimeGenerationFactory()
            await factory2.prepare(
                config=config,
                config_digest="test-digest",
                generation_id=2,
                process=process,
            ...
            resource_names = [r.name for r in candidate._resources]
            assert "dispatch_writer" not in resource_names
        finally:
            await db.disconnect()
