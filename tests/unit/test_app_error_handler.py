...
@pytest.mark.asyncio
async def test_premature_stream_eof_preserves_classification() -> None:
    app = create_app()
    handler = app.exception_handlers[AggregatorError]
    request = Request(
        {
    ...
    response = await handler(request, PrematureStreamEOFError("premature_eof"))  # type: ignore[operator]

    assert response.status_code == 502
    assert response.body == (
        b'{"error":"Upstream stream ended without a valid terminal event '
        b'(premature_eof)","classification":"premature_eof"}'
    )
