...
EggPool dispatch overhead is approaching roughly one second when request shaping / compression is enabled. The current goal is to reduce dispatch latency, CPU usage, and transient memory pressure without removing request-shaping capability, changing routing semantics, weakening fail-closed compression safety, or introducing a Rust extension in this pass.

This plan covers the first five optimization items only:
...
4. Reduce segmentation allocation / serialization / token-estimation overhead.
5. Shorten the serialized selection path and reduce routing-trace write pressure.

The plan intentionally defers Rust / PyO3 acceleration. The implementation should first determine how much latency can be recovered inside the existing Python architecture.
...
The current compression path can run both `analyze_compression()` and `apply_safe_compression()` for the same request. In safe mode this means segmentation is followed by an observe-style analyzer pass and then a mutating applier pass. The applier separately walks the same segments and re-runs transform eligibility / transform logic against actual string leaves.
...
Make the dispatch overhead legible before changing the code path. The existing overhead recorder should expose enough timing fields to answer which phase dominates: JSON decode, context checks, preflight transcode, compression policy resolution, segmentation, compression analysis, compression application, selection lock wait, locked selection work, DB writes, routing trace construction/write, upstream connection wait, and finalizer work.
...
Add a lightweight per-request timing accumulator. Prefer integer nanoseconds from `time.perf_counter_ns()` internally and convert to milliseconds only at reporting boundaries. Avoid dataclass-heavy nested records in the hot path; a small slots dataclass or plain dict with fixed keys is acceptable, but it should not allocate large nested structures per request.
...
- `runtime_publication_ms` if there is a separate publication step below the currently inspected snippet.
...
Add a unit test that verifies disabled compression records no compression spans or zero/none spans without forcing segmentation.
...
Add a dashboard/API test that verifies the new timing payload is accepted when fields are absent, partially present, and fully present.
...
If the full performance marker is too slow for local iteration, at minimum add a focused benchmark-like regression test that can run quickly in CI and documents expected relative relationships rather than absolute hardware-specific timings.
...
Introduce a single-pass safe-mode return shape that can satisfy the fields currently needed by `FinalizationData.compression_observation` and `FinalizationData.compression_result`. There are two acceptable implementation paths:
...
Build the shape descriptor and stable-prefix descriptor from the accumulator data rather than re-walking the segments several times.
...
This change should preserve the existing structural fallback estimates:

- command/tool output repeated line estimate proportional to tokens
- log compaction estimate proportional to tokens
- search compaction estimate proportional to tokens
- base64/blob estimate based on byte/token thresholds
- JSON minify structural fallback for blob-shaped regions
- stack trace fallback for command output
...
- exclude account set
- any static provider/model/protocol fields needed by the routing plan
