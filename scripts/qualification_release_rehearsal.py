#!/usr/bin/env python3
"""Run the deterministic release rehearsal wheelhouse and staged-release rehearsal.
...
validation remain owned by their release workflow implementations.  Release
rehearsal only binds their results to one bounded, secret-free report and adds
the package-index
...
from inspect_release_raw import TARGETS, inspect_raw
from inspect_release_wheel import inspect_wheel
from validate_release_artifacts import ValidationError, validate_manifest
from validate_release_workflow import WorkflowValidationError, validate_workflow

ROOT = Path(__file__).resolve().parents[1]
WORKFLOW = ROOT / ".github/workflows/release.yml"
DEFAULT_JSON = ROOT / "packaging/release/upgrade-rehearsal.json"
DEFAULT_MARKDOWN = ROOT / "artifacts/qualification/release-rehearsal.md"
MANIFEST_SCHEMA = "release-rehearsal.v1"
...
class RehearsalError(RuntimeError):
    """A bounded release rehearsal failure."""
...
class SimpleIndex:
    ...
    def __init__(self, wheelhouse: Path) -> None:
        self.root = Path(tempfile.mkdtemp(prefix="eggpool-release-rehearsal-index-"))
...
def _unsupported_target(wheelhouse: Path, version: str) -> dict[str, str]:
    with tempfile.TemporaryDirectory(
        prefix="eggpool-release-rehearsal-unsupported-"
    ) as value:
...
def _public_style_index(
    ...
    with (
        ...
        tempfile.TemporaryDirectory(
            prefix="eggpool-release-rehearsal-index-download-"
        ) as value,
    ):
        destination = Path(value)
        result = subprocess.run(
...
def _failure_injection_evidence(
    ...
    cases: dict[str, str] = {}
    with tempfile.TemporaryDirectory(
        prefix="eggpool-release-rehearsal-faults-"
    ) as value:
        root = Path(value)
        ...
        corrupt.write_bytes(corrupt.read_bytes() + b"release-rehearsal-corruption")
        try:
    ...
    cases.update(
        {
            ...
            "partial_manager_install": "installer harness",
            "staged_target_self_check": "release artifacts wheel smoke",
            "service_restart_failure": "service transition recovery harness",
...
def _installer_evidence(
    ...
    with tempfile.TemporaryDirectory(
        prefix="eggpool-release-rehearsal-installer-"
    ) as value:
        root = Path(value)
...
def _wheel_smoke(wheel: Path, target_class: str, version: str) -> dict[str, str]:
    try:
        _run(
            [
                sys.executable,
                str(ROOT / "scripts/qualify_release_wheel.py"),
                str(wheel),
                "--target-class",
                target_class,
                "--version",
...
def _transition_evidence(
    *,
    ...
    if target_class != _host_target():
        return {"status": "skipped", "reason": "target does not match this host"}
    with tempfile.TemporaryDirectory(
        prefix="eggpool-release-rehearsal-transitions-"
    ) as value:
        output = Path(value) / "transitions.json"
        command = [
            sys.executable,
            str(ROOT / "scripts/qualify_release_transitions.py"),
            "--python-wheel",
            str(python_wheel),
            "--rust-wheel",
            str(rust_wheel),
        ...
        return {
            ...
            "runner": "scripts/qualify_release_transitions.py",
            "managers": rows,
        }
    ...
    *,
    ...
    return {
        ...
        "qualification_freshness": {
            "status": "pass",
            "basis": (
                "release rehearsal changes release/install orchestration only; "
                "runtime-visible runtime surfaces are unchanged"
            ),
        },
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Staged release rehearsal run",
            "",
            f"Status: **{report.get('status', 'unknown')}**",
            "",
            f"- Release: `{release.get('version', 'unknown')}`",
            f"- Source commit: `{release.get('source_commit', 'unknown')}`",
            f"- Manifest SHA-256: `{release.get('manifest_sha256', 'unknown')}`",
