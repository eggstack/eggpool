#!/usr/bin/env python3
"""Build and validate one release packaging Rust binary wheel."""
...
class BuildContractError(ValueError):
    """A pre-build or post-build release packaging contract failure."""
...
def _preflight() -> str:
    ...
    if cargo.get("version") != _release_version():
        raise BuildContractError(
            "Cargo version does not equal the release catalog native release version"
        )
    if not CARGO_MANIFEST.with_name("Cargo.lock").is_file():
        raise BuildContractError("Cargo.lock is required for a native release build")
...
def _release_version() -> str:
    ...
    if not isinstance(version, str):
        raise BuildContractError("release catalog native release version is missing")
    return version
...
def _host_target() -> str:
    ...
    raise BuildContractError(
        "host is not a qualified release packaging build target; pass an explicitly "
        "qualified --target"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.release_version and args.release_version != version:
            raise BuildContractError(
                "release version does not equal Cargo/release catalog version"
            )
        if args.release_tag and args.release_tag.removeprefix("v") != version:
            raise BuildContractError(
                "release tag does not equal Cargo/release catalog version"
            )
        ...
        if sdists:
            raise BuildContractError("Rust release build emitted an sdist")
        ...
        print(f"release packaging build failed: {error}", file=sys.stderr)
        return 1
