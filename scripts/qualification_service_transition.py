#!/usr/bin/env python3
"""Qualify the deployed Python/Rust service transition on disposable Linux.
...
Example (disposable Linux VM)::
    ...
    sudo -E uv run python scripts/qualification_service_transition.py \
      ...
      --mode personal --output artifacts/qualification/service-transition.json \
...
SERVICE = "eggpool"
MARKER = Path("/var/tmp/eggpool-service-transition-owned")
...
def require_linux() -> None:
    if platform.system() != "Linux":
        raise QualificationError(
            "service transition requires Linux; no host mutation was attempted"
        )
    if os.geteuid() != 0:
        raise QualificationError("service transition requires effective root")
    if Path("/proc/1/comm").read_text(encoding="utf-8").strip() != "systemd":
        raise QualificationError("service transition requires systemd as PID 1")
    missing = [
        name
...
def write_unit(
    ...
    *,
    ...
    content = "\n".join(
        [
            "[Unit]",
            "Description=EggPool service transition qualification",
...
def run_cycle(
    ...
    require_linux()
    preflight(mode)
    root = Path(tempfile.mkdtemp(prefix="eggpool-service-transition-"))
    runner = Runner(root)
    user = f"service-transition-{os.getpid()}"
    user_home = root / "home"
    ...
    try:
        if mode == "personal":
            ...
            MARKER.write_text(
                "service transition disposable ownership marker\n", encoding="utf-8"
            )
            for directory in (
                Path("/etc/eggpool"),
                Path("/var/lib/eggpool"),
