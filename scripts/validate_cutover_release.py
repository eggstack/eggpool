#!/usr/bin/env python3
"""Validate the immutable K001/K002 release identity before a K008 build."""
...
def validate_candidate(
    *, tag: str | None = None, source_commit: str | None = None
    ...
    if not isinstance(version, str) or not VERSION_RE.fullmatch(version):
        raise ReleaseValidationError("catalog cutover version is invalid")

    cargo_package = cast("dict[str, Any]", _read(CARGO).get("package"))
    if cargo_package.get("version") != version:
        raise ReleaseValidationError(
            "Cargo version disagrees with the K001 cutover version"
        )
    ...
    if (
        ...
        raise ReleaseValidationError(
            "Maturin build pin/backend is not the reviewed K002 contract"
        )
    maturin = cast(
    ...
    if tag is not None:
        ...
        if match is None or match.group(1) != version:
            raise ReleaseValidationError(
                "release tag does not match the candidate version"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        print(
            json.dumps(
                validate_candidate(tag=args.tag, source_commit=args.source_commit),
                sort_keys=True,
            )
        )
    except (
        ...
        print(f"K008 release identity validation failed: {error}", file=sys.stderr)
        return 1
    return 0
