#!/usr/bin/env python3
"""Build a K003 wheel/raw pair with pinned, explicit target settings."""
...
class BuildError(ValueError):
    """A deterministic K003 build contract failure."""
...
def _clean_source() -> None:
    ...
    if result.stdout.strip():
        raise BuildError("K003 release builds require a clean source revision")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"K003 build failed: {error}", file=sys.stderr)
        return 1
    print(f"K003 built {wheel.name} and {raw.name}")
    return 0
