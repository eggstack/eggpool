#!/usr/bin/env python3
"""Validate the runtime current-package and historical-tooling boundary."""
...
def validate_package_boundary(
    *, root: Path = ROOT, workflow_text: str | None = None
    ...
    if retirement.get("project_role") != "repository-tooling-only":
        raise PackageBoundaryError(
            "root pyproject.toml must declare repository-tooling-only role"
        )
    ...
    _version(authority.get("native_release_version"), "catalog native_release")
    _version(
    ...
    return {
        ...
        "native_release_version": str(authority["native_release_version"]),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"runtime package boundary validation failed: {error}", file=sys.stderr)
        return 1
    return 0
