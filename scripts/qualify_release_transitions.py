#!/usr/bin/env python3
"""Qualify the release compatibility Python -> Rust -> Python -> Rust package cycle.
...
def _bounded_error(error: BaseException) -> dict[str, str]:
    ...
    if os.environ.get("RELEASE_COMPATIBILITY_DIAGNOSTIC_ERRORS"):
...
def _config(root: Path) -> Path:
    ...
    source = (
        ROOT / "tests/tooling/fixtures/qualification/compatibility.toml"
    ).read_text()
...
def qualify_manager(
    ...
    *,
    ...
    temporary = tempfile.mkdtemp(prefix=f"eggpool-{name}-")
    root = Path(temporary)
    try:
        config = _config(root)
        ...
        if not os.environ.get("RELEASE_COMPATIBILITY_KEEP_ROOT"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        document = {
            "schema_version": "transition-matrix.v1",
