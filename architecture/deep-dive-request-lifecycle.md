...
- A bare or unknown 401 is not credential evidence: it is returned without account disablement, health penalty, or failover. Explicit credential invalidity may disable only the selected account. Deterministic auth/surface/schema mismatch, or weak endpoint-local model rejection when the selected provider catalog knows the model, may reject only the selected wire candidate before downstream handoff; strong model absence remains model-scoped failure behavior.
