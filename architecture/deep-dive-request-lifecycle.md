...
The pre-bound listener is driven by EggServe's HTTP/1 runtime and its direct,
server-owned Tower adapter into EggPool's Axum router. EggServe does not own
application admission policy: the Axum middleware acquires a generation lease
before collecting inference bodies and applies that generation's configured
