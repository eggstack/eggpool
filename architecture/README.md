...
**Provider-bound payload lifecycle (Plans 050/107/114)**: `ProviderBoundRequest` is created from the single client parse and remains authoritative through provider selection and dispatch. The canonical graph is logically immutable. Narrow provider changes use path-level copy-on-write: stream-options normalization copies only the root and affected mapping, while unknown mutators retain the conservative deep-owning path. EggPool-owned path-level transforms, including safe compression, cross the explicit `adopt_provider_payload()` boundary without recursively rematerializing unchanged subtrees. A valid `PreparedTranscode` still uses the conservative ownership path until Plan 115. When no body semantics change, serialization reuses the accepted client bytes byte-for-byte; generation advances and cached bytes are invalidated only for real provider-visible changes. After the selected response is prepared for downstream handoff, dispatch-only raw/parsed/provider buffers are released while scalar accounting remains. See `src/eggpool/request/provider_bound_request.py`, `src/eggpool/request/transform_pipeline.py`, and `tests/unit/test_provider_bound_request.py`.
