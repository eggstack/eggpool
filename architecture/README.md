...
- **Analytics index audit (migration 0053)** removes the unused `request_attempts(status_code, started_at)` analytics index. Provider/model filtered attempt views and retry time-window scans retain their supporting indexes; correctness, recovery, identity, expiry, and retention indexes are not optional dashboard tuning.
...
**Plan 105 — Native transcode parity**: tool translation is baseline protocol
compatibility; the legacy `[transcoder.features].tools` value is retained only
for config compatibility. `ModelCapabilities.transcoding` supplies explicit
static/provider/model facts for native structured output, strict tools,
parallel-tool disabling, and named reasoning efforts. Empty capability lists
are unknown, so native fields are never emitted solely from protocol names.
OpenAI JSON schema maps to Anthropic `output_config.format`, Anthropic JSON
schema maps to OpenAI `response_format`, and strict/parallel controls map only
when the target capability is verified. Unsupported or unverified mappings
remain visible loss warnings and obey the configured loss policy.
...
**Provider-bound payload lifecycle (Plans 050/107/114/115)**: `ProviderBoundRequest` is created from the single client parse and remains authoritative through provider selection and dispatch. The canonical graph is logically immutable. Narrow provider changes use path-level copy-on-write: stream-options and selected-provider budget normalization copy only the root and affected mapping, while unknown mutators retain the conservative deep-owning path. EggPool-owned path-level transforms, including safe compression, and request-local prepared transcode generations cross the explicit `adopt_provider_payload()` boundary without recursively rematerializing unchanged subtrees. A valid unchanged `PreparedTranscode` supplies its existing encoded bytes directly; thinking-control adaptation and synthetic cache synthesis retain one conservative provider-owned full copy where their broad or legacy transforms require it. When no body semantics change, serialization reuses the accepted client bytes byte-for-byte; generation advances and cached bytes are invalidated only for real provider-visible changes. After the selected response is prepared for downstream handoff, dispatch-only raw/parsed/provider buffers are released while scalar accounting remains. Prepared transcodes are never shared across requests. See `src/eggpool/request/provider_bound_request.py`, `src/eggpool/request/transform_pipeline.py`, and `tests/unit/test_provider_bound_request.py`.
...
`PreparedTranscode` (`src/eggpool/transcoder/prepared.py`) captures the request-local translated payload, encoded body, warnings, and deterministic feature fingerprint from preflight. Construction does not recursively freeze or rematerialize the translated graph. The coordinator checks `prepared.is_valid_for(upstream_protocol, features)` before re-encoding; valid unchanged reuse adopts the generation through `ProviderBoundRequest.adopt_provider_payload()` and reuses the existing body and warnings. Provider-specific thinking or cache changes establish a new provider generation through the normal COW/ownership APIs, while retry after serialization continues to use the frozen provider bytes. Falls back to full recompute when thinking controls or feature mismatches are detected. Debug observability lives on `PreparedTranscodeDiagnostics` via `available`, `reused`, and `recompute_reason`.
...
- **Structured**. `FinalizationResult` distinguishes durable terminal state,
  ...
  before a later component fails. Once the lease is released, transient retry
...
- **One owner**. Retryable failures are scheduled by one generation-owned supervisor timer with
...
ID.  Each task:
- waits for both active lease count and terminal-reference count to reach
  zero (via `drain_event`) or the drain deadline;
...
**Event-based drain signaling**: `GenerationLease.release()` and the
generation-owned `RequestFinalizationSupervisor` signal `slot.drain_event`
when both `active_leases` and `terminal_references` reach zero. The
retirement task uses `asyncio.wait_for(slot.drain_event.wait(), ...)` instead
of polling with `asyncio.sleep`. If a live retirement deadline expires while
terminal references remain, the slot stays resident and the worker enters the
existing fail-closed fatal path; releasing the final reference resumes normal
close. Shutdown explicitly permits bounded reference abandonment for startup
repair after process death.
...
An unresolved terminal reference is different: live retirement does not force
close its generation dependencies. It records bounded failure class/stage
diagnostics and invokes the fatal worker handler instead.
...
terminal_references, forced_close, timing, and bounded failure facts),
`retirement_task_count`, and `shutdown_in_progress`. Runtime metrics also
expose `finalization_ownership` with active generation/supervisor counts,
retiring-generation count, total terminal references, oldest retiring age,
blocked status, and redacted last failure class/stage. `GenerationDiagnostics` includes `state`
...
## Generation-Owned Request Finalization (Plans 026 and 080)

Makes selected-attempt cleanup independent of the client request task. Once EggPool has durably created a request, attempt, or reservation and claimed runtime ownership, one retained generation-owned finalization job must own terminal reconciliation until every durable and in-memory obligation has either completed or entered a bounded, observable retry state. The owning generation cannot close while that job retains a terminal reference.

### Design principle

`asyncio.shield()` alone is not ownership. A shielded coroutine may continue after the outer task is cancelled, while the outer task skips subsequent cleanup. Eggpool must retain the finalization task in generation-owned state, observe its completion independently of request waiters, reconcile completion exactly once, and keep bounded retry ownership when durable finalization cannot complete immediately.
...
- `ProviderBoundRequest` (`src/eggpool/request/provider_bound_request.py`) — typed lifecycle object owned by one proxy request; carries `client_payload` (immutable), `provider_payload` (path-level copy-on-write via `mutate_top_level_mapping`, conservative ownership via `set_provider_payload`, or trusted adoption via `adopt_provider_payload`), `provider_bytes` (serialized once), `payload_generation` counter, and `SegmentationValidityKey` for cache reuse.
