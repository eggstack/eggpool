...
**Provider-bound payload lifecycle (Plan 050/107)**: `ProviderBoundRequest` is created from the single client parse and remains authoritative through provider selection and dispatch. The canonical graph is logically immutable; the first provider-bound mutation creates one deep, owned ordinary graph, later transforms mutate that graph, and final serialization operates directly on it. A valid `PreparedTranscode` supplies owned decoded payload and matching bytes. When no body semantics change, serialization reuses the accepted client bytes byte-for-byte; model normalization and every protocol/cache/reasoning mutation disable that path. After the selected response is prepared for downstream handoff, dispatch-only raw/parsed/provider buffers are released while scalar accounting remains. See `src/eggpool/request/provider_bound_request.py`, `src/eggpool/request/transform_pipeline.py`, and `tests/unit/test_provider_bound_request.py`.
