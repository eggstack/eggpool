...
- Canonical request-cost precedence in `RequestFinalizer`: `provider_reported` upstream cost wins; otherwise only trusted local `derived` / `partial` / `exact` values may become canonical. Positive local `estimated` values are routed through `choose_bounded_estimated_cost()` so the lower plausible value between the local estimate and `selected.estimated_microdollars` wins — a generous reservation MUST NOT silently override a tighter local estimate. The structured `cost.reservation_fallback_suppressed` event is emitted when the reservation would otherwise dominate. The `estimated`-cost floor still applies (canonical `estimated` cost is never lower than the reservation) so quota accounting never reports less spend than the routing layer pre-reserved.
