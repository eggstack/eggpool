...
Async SQLite with WAL mode, single-connection serialization, and 51 numbered schema migrations. `db/connection.py` wraps `aiosqlite` with `BEGIN IMMEDIATE` transactions, per-task ContextVar-based nesting detection, connection invalidation/recovery, and a `DatabaseLifecycleState` state machine. All DML runs inside `async with db.transaction():`. `db/repositories.py` contains `AccountRepository`, `ProviderRepository`, `RequestRepository`, and others. `db/dispatch_repository.py` handles dispatch persistence. `db/rollup_repository.py` manages usage rollups. `db/recovery.py` implements `DatabaseRecoveryController` with single-flight recovery and bounded retry. Schema lives in `db/schema/` with 51 numbered SQL files and a `checksums.json` manifest.
...
Circuit breaker-based health tracking for accounts and models. `HealthManager` (`health/health_manager.py`) manages per-account health with consecutive failure counting, circuit breaker probe-slot acquisition, operator-initiated disable/enable, and cooldown periods. `health/circuit_breaker.py` implements per-account circuit breaker logic. `health/writable_probe.py` provides `DatabaseWritableProbe` — a real SQLite write probe cached for `/readyz` (never performs a write on the request path). `failure/` implements typed failure effects: `classify_failure_effects()` centralizes consequences; `FailureEffectApplier` applies backoff, suppression, and quarantine decisions. `ModelQuarantine` (`failure/quarantine.py`) is a bounded state machine with corroboration before terminal withdrawal. `health/backoff.py` implements per-account backoff logic.
...
Typed failure effects and bounded model quarantine. `failure/classifier.py` (`classify_failure_effects()`) centralizes consequences of upstream failures. `failure/effects.py` defines failure effect types. `failure/applier.py` applies effects (backoff, suppression, quarantine). `failure/quarantine.py` implements `ModelQuarantine` — a bounded state machine with corroboration before terminal withdrawal. `failure/signal.py` and `failure/signal_extract.py` extract failure signals from upstream responses. `failure/observation.py` records failure observations.
