...
`ProviderBoundRequest` is the single provider-payload authority after client parsing. Generation-aware path-level copy-on-write handles narrow mutations, `adopt_provider_payload()` accepts EggPool-owned transformed graphs and request-local prepared transcode generations, conservative setters protect unknown graphs, and one final serialization cache freezes the request before dispatch. Cross-protocol encoders receive a read-only `Mapping` and return a fresh target graph that is adopted directly, without a defensive source deepcopy or second recursive ownership pass. Valid unchanged prepared transcodes reuse their encoded body exactly; later provider-specific changes establish a new provider generation. `ProxyRequestContext` retains original client bytes separately and carries the provider-bound object without a second body mirror.

### Thinking-Control Adaptation Ownership
`adapt_thinking_controls()` accepts a read-only `Mapping[str, Any]` source and builds its own shallow-copied working root. The coordinator's `_adapt_provider_thinking_controls()` adopts changed results through `ProviderBoundRequest.adopt_provider_payload(reason="thinking_control")` rather than routing them through the conservative `replace_provider_payload()` helper. No-op adaptation leaves `payload_generation` unchanged and preserves the cached provider bytes. The `mutate_provider_payload()` arbitrary mutator was removed (Plan 121) because no production caller remained; the explicit narrow ownership primitives (`mutate_top_level_mapping`, `adopt_provider_payload`, `set_provider_payload`) cover the narrowed surface. The former `provider_payload_copy()` transcode seam was removed in Plan 124; `replace_provider_payload()` remains only as a conservative conditional ownership primitive for compatibility callers.
