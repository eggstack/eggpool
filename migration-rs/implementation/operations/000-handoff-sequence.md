...
3. O003 — implement daemon serve, stop/restart, rehash, runtime-status, croncheck and ensure-running on O002/M8 (**closed**).
...
8. O008 — implement latest/exact update resolution, staged verified replacement, version behavior, and update-checker M8 task (**closed; c9b63d5**).
9. O009 — implement deploy systemd/cron/backup-cron/logrotate/all, reviewed Rust candidate install artifacts, and uninstall (**closed; 27310ff**).
10. O010 — run full M9 differential/fault/security/data-loss/deployment qualification and close M9 (**closed; see closure/operations/010-status.md**).
...
Only `migration-rs/registry.md` authorizes implementation. O010 is closed; M10 is eligible for its own planning/implementation review.
